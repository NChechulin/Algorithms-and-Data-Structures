...
impl Card {
    pub fn from_string(s: String) -> Card {
        ...
        let suit = &tokens[0];
        let rank = &tokens[1];
        Card { suit: Suit::from_string(suit), rank: Rank::from_string(rank) }
    }
}
...
mod tests {
    ...
    #[test]
    fn correct_parsing() {
        assert_eq!(Card::from_string("CLUBS#ACE".to_string()), Card { suit: Suit::Clubs, rank: Rank::Ace });
        assert_eq!(Card::from_string("HEARTS#JACK".to_string()), Card { suit: Suit::Hearts, rank: Rank::Jack });
        assert_eq!(Card::from_string("SPADES#9".to_string()), Card { suit: Suit::Spades, rank: Rank::Nine });
        assert_eq!(Card::from_string("DIAMONDS#QUEEN".to_string()), Card { suit: Suit::Diamonds, rank: Rank::Queen });
    }
